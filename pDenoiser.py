...
def denoise(signal, start_index, end_index, threshold_value, threshold_type='hard'):
    denoised_signal = signal.copy()
    denoised_signal[start_index:end_index] = threshold(denoised_signal[start_index:end_index], threshold_value, threshold_type)
    return denoised_signal

# Define a function to parallelize the denoising process
def parallel_denoise(signal, num_processes, threshold_value, threshold_type='hard'):
    pool = mp.Pool(num_processes)
    chunk_size = len(signal) // num_processes
    
    # Create a list of arguments for the denoise function
    args_list = [(signal[i:i + chunk_size], i, i + chunk_size, threshold_value, threshold_type) for i in range(0, len(signal), chunk_size)]
    
    # Apply denoising to signal chunks in parallel
    denoised_chunks = pool.starmap(denoise, args_list)
    pool.close()
    pool.join()
    
    # Concatenate the denoised chunks to get the final denoised signal
...
if __name__ == '__main__':
    ...
    def apply_wavelet(signal, wavelet_schoize):
        ...
        if wavelet_choice == '1':
            ...
            approx, detail = sym8(signal)  # sym5(Symlet 8) wavelet
        elif wavelet_choice == '6':
            approx, detail = coif5(signal)  # coif5(Coiflet 8) wavelet
